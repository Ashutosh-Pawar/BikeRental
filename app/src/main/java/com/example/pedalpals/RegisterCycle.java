...
public class RegisterCycle extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    EditText reg_no,  color, condition;
    Spinner price,model,location;
    Button register;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int upperbound=200;
        int randomVar=rand.nextInt(upperbound);
        reg_no.setText(""+randomVar);

        prefs = this.getSharedPreferences("PedalPals", 0);
        username = prefs.getString("username", "");


        ArrayAdapter<CharSequence> madapter=ArrayAdapter.createFromResource(this,R.array.modelList, android.R.layout.simple_spinner_dropdown_item);
        madapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        model.setAdapter(madapter);

        ArrayAdapter<CharSequence> padapter=ArrayAdapter.createFromResource(this,R.array.priceList, android.R.layout.simple_spinner_dropdown_item);
        madapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    ...
    private void registerCycle(final int randomVar) {
        register.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int pos=model.getSelectedItemPosition();
                String array[]=getResources().getStringArray(R.array.modelList);
                String modelname=array[pos];

                int pos1=price.getSelectedItemPosition();
                String[] array1 =getResources().getStringArray(R.array.priceList);
                String prices=array1[pos];

                if(prices.equals("") || modelname.equals("") || color.getText().toString().trim().equals("") ||
                    ...
                    Integer p=Integer.valueOf(prices);


                    boolean isInserted = db.insertData_Cycle(randomVar,
                            modelname,
                    ...
                    if (isInserted) {
                        Toast.makeText(RegisterCycle.this, "Bike Registered", Toast.LENGTH_SHORT).show();
                    } else {
